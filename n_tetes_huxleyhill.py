...
k01 = lambda s : np.exp(-((s-50)/50)**2)    #direct transition rates
k10 = lambda s : 1

w0 = lambda s : 0.1     #energy landscape and derivatives for detached head
dw0 = lambda s : 0

w1 = lambda s : 10*((s-50)/50)**2      #energy landscape and derivatives for attached head
dw1 = lambda s : 0.4*(s-50)/50
...
alpha = [[0 for t in range(nsteps)] for i in range(N)]      #actual description of the stochastic process
...
for t in range(nsteps - 1):
  ...
  #s dynamics
  s[t + 1] = s[t] + F*dt - (dt/v)*sum([alpha[i, t]*dw1[s[t]] for i in range(N)])
